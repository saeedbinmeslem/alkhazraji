...
import { validateTaxonomy, createSlug } from 'shared/taxonomy';

// ─── Ref-counted scroll lock ──────────────────────────────────────────────────
// Uses a counter stored on the body element so that if two taxonomy modals were
// ever open simultaneously, one closing would not prematurely restore scrolling.
...
export default function TaxonomyForm({ item, type, onSave, onCancel }) {
  const [name, setName] = useState(item?.name || '');
  const [slug, setSlug] = useState(item?.slug || '');
  // Track whether the user has manually edited the slug field.
  // In edit mode we start as true so the existing slug is never auto-overwritten.
  // In create mode we start as false so the slug auto-generates from the name.
  const [slugManuallyEdited, setSlugManuallyEdited] = useState(!!item);
  ...
  // Auto-generate slug from name while the user types — but only for new items
  // and only while the slug has not been manually customized.
  // Once slugManuallyEdited is true, name changes no longer overwrite the slug.
  useEffect(() => {
    if (!item && !slugManuallyEdited) {
      setSlug(createSlug(name));
    }
  }, [name, item, slugManuallyEdited]);

  const handleSubmit = (e) => {
    e.preventDefault();
    setError(null);
    
    // Map taxonomy type to Arabic label for UX
    const typeLabel = {
      category: 'قسم',
      brand: 'ماركة',
      collection: 'تشكيلة'
    ...
    <div style={{ background: 'rgba(0,0,0,0.5)', position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <div style={{ background: '#141414', padding: '32px', borderRadius: '24px', width: '100%', maxWidth: '500px', border: '1px solid var(--border-color)', boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div>
            <label style={{ display: 'block', color: 'var(--text-muted)', marginBottom: '8px', fontSize: '0.9rem' }}>المعرف (Slug)</label>
            <input 
              type="text" 
              value={slug} 
              onChange={(e) => {
                // First manual keystroke in the slug field locks auto-generation off.
                // Subsequent name changes will no longer overwrite this field.
                setSlugManuallyEdited(true);
                setSlug(e.target.value);
              }} 
              style={{ width: '100%', padding: '12px 16px', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-color)', borderRadius: '12px', color: '#fff', outline: 'none' }}
              placeholder="معرف الرابط (e.g., my-category)"
            />
          </div>
